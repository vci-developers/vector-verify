...
`isProgramsError` guard, so every consumer inside the provider gets a guaranteed
`string` — never `string | undefined`. `useCountry()` throws if called outside
the provider. This is the first React Context used for shared app data in this
codebase; every other cross-component data need is solved with plain props or
TanStack Query's own cache dedup (each consumer calling the same `useGet…` hook
independently).
...
- **Lift `country` to `operations-page-client.tsx` and pass it as a prop
  ...
  follow the established one, for a value only one of four Operations tabs uses.
- **Restructure `SiteMap`/`DeviceMap` into children-accepting shells
  ...
  here because `country` is the _only_ pass-through prop on those components —
  ...
  mean rewriting otherwise-correct components to move the problem, not solve it.
- **Context with a plausible default value, e.g. `createContext('Uganda')`
  (rejected).** A default masks a missing provider silently — the exact failure
  mode this ADR exists to fix (a hardcoded `'Uganda'` shipped undetected). A
  missing provider must fail loudly in development, not degrade to a
  wrong-but-plausible country.
- **Context typed `string | undefined`, consumers self-gate (rejected).** Keeps
  the provider trivial but re-spreads undefined-handling across every consumer —
  the same fragmentation Context was meant to centralize. Gating once at the
  mount site (behind the existing pending/error guard) means every consumer
  downstream is unconditionally correct.
...
- This is a precedent, not a one-off: the next value that is genuinely
  ...
  call; only true pass-through-with-no-intermediate-use earns a scoped Context.
  `programId` was evaluated and rejected for this Context (Q6): its chain is one
  hop and both endpoints consume it directly, so it stays a prop.
