...
- Each feature keeps one parser-definitions module (the documented nuqs
  `searchParams.ts` pattern): ~15 lines of
  `parseAsStringLiteral([...]).withDefault(...)` per feature. The month param
  becomes one custom `createParser` reusing the existing parse/format logic.
