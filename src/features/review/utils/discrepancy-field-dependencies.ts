...
export function calculateDependentFields(
  ...
  if (
    fieldKey === FIELD_KEYS.WAS_IRS_CONDUCTED &&
    value.toLowerCase() === 'no'
  ) {
    const monthsSinceIrsField = allFields.find(
      f => f.key === FIELD_KEYS.MONTHS_SINCE_IRS,
    );
...
export function shouldDisableField(
  ...
  if (llinDependentFields.includes(fieldKey)) {
    ...
    return (
      numLlinsAvailable === '0' || numLlinsAvailable?.toLowerCase() === 'zero'
    );
  }
