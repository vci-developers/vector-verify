...
import { AbdomenStatusLabels, SexLabels, SpeciesLabels } from '@/lib/domain/reference/labels';
...
const ABDOMEN_STATUS_VALUES = Object.values(AbdomenStatusLabels) as [string, ...string[]];
...
export const isSexEnabled = (species?: string) => species !== SpeciesLabels.NON_MOSQUITO;
export const isAbdomenStatusEnabled = (species?: string, sex?: string) =>
  isSexEnabled(species) && sex !== SexLabels.MALE;

export const AnnotationFormSchema = AnnotationBase.superRefine((formFields, context) => {
  ...
  if (isAbdomenStatusEnabled(formFields.species, formFields.sex) && !formFields.abdomenStatus) {
    context.addIssue({
      code: 'custom',
      path: ['abdomenStatus'],
